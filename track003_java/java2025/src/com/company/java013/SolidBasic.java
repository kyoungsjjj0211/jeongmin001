...
1. S : (한클래스는 하나만 1:1) 한글래스는 한가지 일만 (쿠기반죽은 반죽만, 쿠기굽기는 굽기만)
2. O : (수정말고 추가만)새로운 쿠기 쉽게 추가
3. L : (다형성 : 같은방식으로 처리)모든쿠기는 같은 방식으로 만들기
...
//1. S : 단일책임의 원칙
class CookieMaker{
	public void bakeCookie(String type) {System.out.println(type + "쿠기를 구워요!");}
}

//2. O : 개방-폐쇄의 원칙 (새로운코드(쿠기)를 추가할수 있지만, 기존코드 건들지 마세요!)
...
//3. L : 리스코프 치환 ,다형성(어떤쿠키든 Cookie 인터페이스로 바꿔써도 문제 없어요! 부모 = 자식)
class CookieFactory{
...
class SimpleCookieMake implements SimpleCookie{
	@Override public void make() { System.out.println("");}
}

//5. 의존역전원칙 D- CookieFacotry는 구체적인 쿠키가 아니라 추상적인 Cookie에 의존
class CookieShop{
