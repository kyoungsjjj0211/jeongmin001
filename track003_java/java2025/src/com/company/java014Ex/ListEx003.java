...
class IceCreamDto{
	...
	public IceCreamDto(String name, int price) {this.name =name;this.price = price;}
...
public class ListEx003 {
	public static void main(String[] args) {
		...
		while(true) {
			...
			System.out.println("3 아이스크림 제거");
			System.out.println("4 아이스크림 존재 확인");
			System.out.println("5 총 아이스크림 개수");
			...
			switch(choice) {
				case 1 :
					System.out.println("아이스크림 이름 : "); String name = scanner.nextLine();
					System.out.println("가격 :"); int price = scanner.nextInt();
					scanner.nextLine(); iceCreamList.add(new IceCreamDto(name,price));
					...
					if (iceCreamList.size() == 0) {
						System.out.println("아직 등록된 아이스크림이 없습니다.");
						   ...
						   for (int i = 0; i < iceCreamList.size(); i++) {
	                            System.out.println(iceCreamList.get(i).toString());
	                        }
	                    }
	                    break;
				case 3: 
					System.out.print("🗑️ 제거할 아이스크림 이름: ");String removeName = scanner.nextLine();
					int find= -1;
					//for(int i=0; i<list.size(); i++) { if(list.get(i).getName().equals(rname)) {find=i;break;}}
                    boolean removed = iceCreamList.removeIf(i -> i.getName().equalsIgnoreCase(removeName));
                    System.out.println(removed ? "🧹 제거 완료!" : "❌ 해당 아이스크림이 존재하지 않습니다.");
                    break;
                    
				case 4:
					 System.out.print("🔍 확인할 아이스크림 이름: "); String checkName = scanner.nextLine();
					 //boolean exists = false; for(IceCreamDto ice : list){if(ice.getName().equals(fname)) {exists=true;}   
					 boolean exists = iceCreamList.contains(new IceCreamDto(checkName, 0));
	                    System.out.println(exists ? "✅ 존재합니다!" : "❌ 없습니다!");
	                    break;
				case 5:
					System.out.println("📦 총 아이스크림 개수: " + iceCreamList.size());
                    break;
