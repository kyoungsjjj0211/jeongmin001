...
|------------------|----------------------------------|
|기술스택|	설명|
|**Git & GitHub	Git**|은 분산형 버전 관리 시스템으로, 여러 개발자가 동시에 작업해도 충돌 없이 코드를 관리할 수 있게 해줘요. GitHub은 이를 기반으로 협업을 가능하게 하는 플랫폼이고요. 브랜치 전략(예: Git Flow)과 충돌 해결 능력은 팀 프로젝트에서 필수입니다. 특히 CI/CD(지속적 통합/배포) 흐름과 연결되면 자동화된 테스트와 배포까지 이어져 실무 효율이 극대화돼요. </strong></span>|
|**Markdown**|	개발 문서를 작성할 때 많이 쓰이는 경량 마크업 언어예요. README 파일, 위키 문서, 기술 블로그 등에서 사용되며, 정보를 구조화하고 가독성 있게 표현하는 능력이 중요합니다. 유지보수성과 협업 효율을 높이는 데 큰 역할을 해요. </strong></span>|
|**VS Code**|	마이크로소프트에서 만든 가볍고 강력한 코드 편집기입니다. IDE의 동작 원리(예: 코드 분석, 디버깅, 확장 기능 등)를 이해하면 생산성을 높이고 실무에서 효율적으로 개발할 수 있어요. 플러그인 활용이나 자동화 설정은 특히 반복 작업을 줄이는 데 유용하죠.</strong></span>|
|**HTML5**|	웹 페이지의 구조를 설계하는 언어입니다. DOM(Document Object Model) 트리를 이해하면 자바스크립트와의 연동이 쉬워지고, 시맨틱 태그를 활용하면 검색엔진 최적화(SEO)와 접근성 향상에도 도움이 돼요.</strong></span>|
|**CSS3**|	웹 페이지의 스타일을 담당하는 언어예요. 박스 모델, Flexbox, Grid 같은 레이아웃 알고리즘을 이해하면 반응형 웹 디자인을 구현할 수 있고, 사용자 경험(UX)을 높이는 데 핵심적인 역할을 해요.</strong></span>|
|**AI 프롬프트	**| ChatGPT 같은 AI 모델을 다룰 때 사용하는 입력 방식이에요. 자연어 처리(NLP)와 모델의 응답 구조를 이해하면 프롬프트 엔지니어링을 통해 원하는 결과를 더 정확하게 얻을 수 있어요. 반복 작업을 자동화하거나, 문서 요약·코드 생성 등 다양한 실무에 활용 가능하죠.</strong></span>|
