...
- Controller/Service에서 현재 사용자로 id로 db조회 응답
...
■ 2. JWT구조
1. Header → 토큰의 머릿말 (이 토큰은 HS256 알고리즘으로 서명했어)
```json
  { "alg": "HS256", "typ": "JWT" }
  ```
...
```json
   ...
   ```
3. Signature → 토큰의 도장( 서버만 아는 비밀키로 찍은 도장 → 위조)
```json
...
1. Refresh Token은 HttpOnly 쿠키에 저장 → 
               JS 접근 불가, Axios가 자동 포함(withCredentials: true).  
   - axios 인스턴스에서 withCredentials: true 설정 OK.
   - 프론트 코드에서 Refresh Token을 직접 건드리지 않음 OK.

2. Access Token은 CSR에서는 localStorage/쿠키에 저장, 
               SSR에서는 서버에서 req.cookies를 읽어 처리.  
...
◎3. 흐름도 + 피그마
◎4. 프로젝트 깃허브에 올리기 / back , front (x팀장 : 팀원들은 내려받아서 테스트)
--------------------------------------------------------------------------- [1차]
◎5. ERD
   1) 주요기능 - 
   2) 기능 또는 맞춰서 테이블 (상담)
