...
it('signup failure', async () => {
 ...
 it('login', async () => {
  ...
  it('updateNickname', async () => {
    api.patch.mockResolvedValue({data : { id: 1 , nickname:'new'} });  // 서버에서 받아오는 값
    const dispatched = [];
    await runSaga({dispatch: (a) => dispatched.push(a) }  , updateNickname , { payload: {userId:1 , nickname:'new'} }  ).toPromise();
    expect(dispatched).toContainEqual(  updateNicknameSuccess({user:{id:1, nickname:'new'} }) );
  });

  it('updateNickname failure', async () => {
    api.patch.mockRejectedValue({ message: 'fail' });   // 서버에서 받아오는 값
    const dispatched = [];
    await runSaga({ dispatch: (a) => dispatched.push(a) }, updateNickname, { payload: {userId:1 , nickname:'new'} }).toPromise();
  ...
  it('updateProfileImage', async () => {
    api.post.mockResolvedValue({data:{ id:1, ufile:'file'} });  // 서버에서 받아오는 값
    const dispatched = [];
    //saga 에서 직적 실행 → dispatch
    //첫 번째 인자 : dispatch , 두번째 인자 : 실행할 saga, 세번째 인자 : saga에 전달할 action객체
    await runSaga({dispatch: (a) => dispatched.push(a) }  , updateProfileImage , { payload: {} }  ).toPromise();
