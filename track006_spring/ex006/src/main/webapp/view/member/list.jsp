...
$(function(){             
	userList();  //유저 전체리스트
	userSelect(); //유저 한명분의 정보
	userUpdate(); //유저 정보수정
	userDelete(); //유저 정보삭제
});
function userList(){ //필수 조건 $.ajax ({ url:, type: , success, error})
	$.ajax({
		url:"selectAll",
		type:"GET", //GET, POST
...
function userListResult(json){console.log(json); //json으로 받아온다
	$(".userTable tbody").empty(); //테이블의 tbody
	let total = json.length; //전체갯수확인
	
	$.each( json , function(idx, user){ // json(전체), function( idx = index 순번 , user 한개씩)  
		$("<tr>") //tr태그 만들기 				//td태그 하나당 하나만 표시되도록
			.append($("<td>").html(total-idx) ) //데이터 붙이기 (append-끝에 붙이기) / (html-reset  안에 데이터가 있으면 지우고 넣기)
			...
			.appendTo(".userTable tbody"); //데이블의 tbody 에 tr붙이기
	});
}
function userSelect(){
	// 수정버튼 클릭시 - 해당유저의 정보를 form 태그안에 넣기
...
function userUpdate(){
	$("#updateUser").on("click" , function(){
		...
		let appUserId = $(".userhidden_no .hidden_id").val(); //해당 번호의 유저를 만들어둔 영역에서 히든 id를 찾아오세요
		$.ajax({
			url:"updateAdmin", //updateAdmin
			type:"POST", //수정할 값 POST
			data:{mbtiTypeId:mbtiTypeId, appUserId:appUserId}, //mbtiTypeId, appUserId 셋팅 -> json 
			success:function(json){
				userList();}, //고객님 잘 받으셨나요? 
			error:function(){alert("error");}
...
function userDelete(){
	//$(".deleteUser").on(); - 오류가능성이 있을수 있음....... (userList는 ajax로 처리해서 가져와야함.) 
	$("body").on("click" , ".deleteUser", function(){ //deleteUser의 버튼을 클릭했을때
		...
		if(confirm(email + "유저를 삭제하시겠습니까?") ){
			$.ajax({
				...
				data:{appUserId:appUserId},
				success:function(json){ 
				userList(); }, //{if(json.result ==1){userList();}} / 외래키로 안되는경우 {if(json.result ==1){userList();} else{alert("해당유저의 글이 남아있습니다.");}
				error:function(){alert("error");}
			});		
		}
	});
