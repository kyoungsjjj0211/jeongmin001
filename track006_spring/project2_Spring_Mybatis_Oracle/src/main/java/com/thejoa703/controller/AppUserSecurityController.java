...
@PreAuthorize("isAnonymous()")//회원가입 기능 아무나 진입 가
...
public String fail(HttpServletResponse response, RedirectAttributes rttr) {
	...
	return "redirect:/security/login"; }
//마이페이지
@RequestMapping("/mypage") //마이페이지 -로그인정보 Principal 1회용
public String mypage(Principal principal, Model model) {
	model.addAttribute("dto",service.selectEmail(principal.getName())); //## principal 에있는 로그인정보를 getName 유저 정보를 가져온다.
...
@RequestMapping(value="/update" ,method=RequestMethod.POST , headers=("content-type=multipart/*")) //회원가입기능
...
@PreAuthorize("isAuthenticated()") //로그인을 했다면
@RequestMapping(value="/delete" ,method=RequestMethod.POST) //회원가입기능
